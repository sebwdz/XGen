...
std::vector<ModuleClass*> const  &ModuleClass::get_links() const
{
    return (m_links);
}
...
void        ModuleClass::move()
{
    ...
    for (it = 0; it != m_obj.size(); it++)
    {
        if (m_obj[it]->get_type() & TYPE_MOVABLE)
        {
            ...
            CAST(Movable*)(m_obj[it])->get_move_line()->interact(this, PARENT);
            get_move_line()->interact(CAST(Movable*)(m_obj[it]), SON);
        }
    }
    for (it = 0; it < m_links.size(); it++)
        m_moveLine.interact(m_links[it], LINK);
    Movable::move();
}
