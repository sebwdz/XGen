...
float      Skeleton::get_angle(std::pair<float, float> pt)
{
    ...
    tmp = sqrt(vct.first * vct.first + vct.second * vct.second);
    if (tmp)
...
void    Skeleton::sort(AN_POINT_LIST &lst)
{
    AN_POINT_LIST::iterator                 it;
    AN_POINT_LIST::iterator                 tmp;
    std::pair<float, std::pair<float, float> > cp;
    char                            cnt;
    ...
    while (cnt)
    {
        ...
        for (it = lst.begin(); it != lst.end(); it++)
        {
            ...
            if (it->first < tmp->first ||
                    (it->first == tmp->first && it->second.first < tmp->second.first))
            {
                cp = *tmp;
                *tmp = *it;
                *it = cp;
