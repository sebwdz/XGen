...
void        Environment::show(sf::RenderWindow  *win)
{
    ...
    for (int it = 0; it < 3; it++)
    {
        for (int v = -(it + 1); v <= (it + 1); v++)
        {
            ...
            if (tmp.first > 20 || tmp.second > 20 || tmp.first < 0 || tmp.second < 0)
                 continue;
...
void        Environment::exec()
{
    ...
    m_pos.first = m_pos.first > 20 ? 20 : m_pos.first < 0 ? 0 : m_pos.first;
    m_pos.second = m_pos.second > 20 ? 20 : m_pos.second < 0 ? 0 : m_pos.second;
    if (m_map[m_pos] == FOOD)
        m_food += 1;
    m_map[m_pos] = CELL;
