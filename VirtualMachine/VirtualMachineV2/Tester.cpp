...
void            Tester::exec()
{
    ...
    while (cycle < 3000)
    {
        ...
        if (!(cycle % 100))
        {
            ...
            if (max < 100.0 / (float)((_time[1].tv_sec + _time[1].tv_usec / 1000000.0) - (_time[0].tv_sec + _time[0].tv_usec / 1000000.0)))
                max = 100.0 / (float)((_time[1].tv_sec + _time[1].tv_usec / 1000000.0) - (_time[0].tv_sec + _time[0].tv_usec / 1000000.0));
    ...
    std::cout << "last " << last << " c/s   " << prev[0] / 100 << " instrs/c     " << ( prev[0] ) / ( prev[1] * 1000 ) << " * 10⁶ instrs/s" << " real : " << (prev[0] / ( t * 1000 )) << " * 10³ instrs/s" << std::endl;
