...
main<(
	set (!w nil (:get_nbr(#str_av^0)))
	set (!h nil (:get_nbr(#str_av^1)))
	set (#complexity nil (:get_nbr(#str_av^2)))
	or ((inf ((!w 5)) inf ((!h 5)))(
			echo ("waiting for width, height, complexity\n")
			ret (1)
	))
	cp (!lab call (:generate 0 (!w !h !c)))
	call (:solve 0 (!lab))
	call (:show 0 (!lab))
	ret (0)
)>
